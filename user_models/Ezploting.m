...
for i = 1:length(pks)
    text(locs(i), pks(i), sprintf('(%.2f, %.2f)', locs(i), pks(i)), 'VerticalAlignment', 'bottom');
end
x= 1:length(Eztr);
...
for i = 1:length(minima)
    text(minima_locations(i), minima(i), sprintf('(%.2f, %.2f)', minima_locations(i), minima(i)), 'VerticalAlignment', 'bottom');
end
xlabel('Time (1e-11)');
