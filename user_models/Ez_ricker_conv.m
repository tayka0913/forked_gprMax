...
% 기존 Ez와 windowed 리커파, 컨볼루션한 Ez를 순차적으로 플롯
figure;
subplot(3,1,1);
plot(time, Eztr);
hold on;
...
grid on;
subplot(3,1,2);
plot(t(index_range), ricker_windowed);
hold on;
xlabel('Time (seconds)');
ylabel('ricker wavelet');
title('Windowed Ricker Wavelet');
grid on;
subplot(3,1,3);
hold on;
xlabel('Time (seconds)');
ylabel('Ez,  Field Strength (V/m)');
title('Ez convolution with Ricker');
grid on;
