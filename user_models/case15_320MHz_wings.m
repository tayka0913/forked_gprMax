...
Ez = h5read('case15_320MHz.out', '/rxs/rx1/Ez');
Eztr = Ez.';

Ez_Lside = h5read('case15_320MHz_Lside.out', '/rxs/rx1/Ez');
Eztr_Lside = Ez_Lside.';
Ez_Rside = h5read('case15_320MHz_Rside.out', '/rxs/rx1/Ez');
Eztr_Rside = Ez_Rside.';
% 시간축 생성
t = dt:dt:t_max; 
...
end_index2 = find(time <= 31e-9, 1, 'last');
Eztr2= Eztr;
...
% figure;
% subplot(3, 1, 1);
% plot(time, Eztr, '-', 'LineWidth', 2, 'Color', [0.2 0.2 0.8]);
% xlabel('Time (s)');
% ylabel('Ez');
% title('main에서 수신된 신호');
% 
% subplot(3, 1, 2);
% plot(time, Eztr_Lside, '-', 'LineWidth', 3, 'Color', [1 0 0]); % 빨간색 굵은 실선으로 그리기
% hold on; % 겹치도록 유지
% plot(time, Eztr, '-', 'LineWidth', 2, 'Color', [0.2 0.2 0.8]); % 파란색 실선으로 메인 신호 겹쳐 그리기
% hold off; % 그리기 종료
% xlabel('Time (s)');
% ylabel('Ez');
% title('Lside에서 수신된 신호');
% 
% subplot(3, 1, 3);
% plot(time, Eztr_Rside, '-', 'LineWidth', 3, 'Color', [1 0 0]); % 빨간색 굵은 실선으로 그리기
% hold on; % 겹치도록 유지
% plot(time, Eztr, '-', 'LineWidth', 2, 'Color', [0.2 0.2 0.8]); % 파란색 실선으로 메인 신호 겹쳐 그리기
% hold off; % 그리기 종료
% xlabel('Time (s)');
% ylabel('Ez');
% title('Rside에서 수신된 신호');
% hold on;
% hold off;
% fontsize(14,"points")
...
figure;
subplot(3, 1, 1);
plot(time, full_norm, 'b', 'LineWidth', 2);
xlabel('Time (s)');
ylabel('Ez');
title('정규화된 main에서 수신된 신호');

subplot(3, 1, 2);
plot(time, full_norm_Lside, 'r', 'LineWidth', 3);
xlabel('Time (s)');
ylabel('Ez');
title('정규화된 Lside에서 수신된 신호');

subplot(3, 1, 3);
plot(time, full_norm_Rside, 'r', 'LineWidth', 3);
xlabel('Time (s)');
ylabel('Ez');
