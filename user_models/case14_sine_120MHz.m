...
sinewaveform = zeros(size(t)); % 초기화
for i = 1:length(t)
    if t(i) < 1/f_peak % 처음 한 주기동안
        sinewaveform(i) = sin(2*pi*f_peak*t(i));
    end
end
...
Ez = h5read('case14_sine_120MHz.out', '/rxs/rx1/Ez');
iteration = 25440;
Time = 300e-9;
...
% 수신2 생성
end_index2 = find(time <= 168e-9, 1, 'last'); % 10 ns의 인덱스 찾기
Eztr2= Eztr;
Eztr2(1:end_index2) = 0;
% 수신3 생성
end_index3 = find(time <= 197e-9, 1, 'last'); 
Eztr3= Eztr;
...
end_index2 = find(time <= 168e-9, 1, 'last'); 
...
figure;
subplot(3, 1, 1);
plot(time, sinewaveform, 'b', 'LineWidth', 2);
xlabel('Time (s)');
ylabel('ricker');
...
title('Ez normalized');
hold on;
scatter(max_time1,full_norm(time==max_time1),100,"black","filled")
scatter(t_calculation1,y_t_calculation1,100,[0 0.4470 0.7410],"filled")%파란색
scatter(t_calculation2,y_t_calculation2,100,[0.8500 0.3250 0.0980],"filled")%황토색
